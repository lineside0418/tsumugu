...
import { PenLine, Eye, Settings2, Download, Moon, Sun, CheckCircle2, X, Image as ImageIcon, Loader2, Bold, Italic, Minus, Copy, Check, Menu } from 'lucide-react';
import { marked } from 'marked';
import DOMPurify from 'dompurify';
import { toPng } from 'html-to-image';
import { useEditorStore } from './store';

// srcフォルダ直下に配置したオリジナルロゴをインポート
...
function App() {
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-[#fcfcfc] dark:bg-[#191919] text-gray-900 dark:text-[#e5e5e5] transition-colors duration-300 relative flex flex-col">
      
      {/* PC版ヘッダー */}
      <header className="hidden sm:flex h-16 px-6 items-center justify-between z-10 border-b border-gray-100 dark:border-neutral-800/50 bg-white/80 dark:bg-[#191919]/80 backdrop-blur-md relative">
        ...
        <div className="absolute left-1/2 transform -translate-x-1/2 flex bg-gray-100 dark:bg-neutral-800/80 p-1 rounded-xl shadow-inner">
          <button 
            ...
            className={`flex items-center space-x-2 px-5 py-2 rounded-lg font-bold text-sm transition-all duration-200 ${mode === 'edit' ? 'bg-white dark:bg-[#252525] text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
          >
          ...
          <button 
            ...
            className={`flex items-center space-x-2 px-5 py-2 rounded-lg font-bold text-sm transition-all duration-200 ${mode === 'preview' ? 'bg-white dark:bg-[#252525] text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
          >
      ...
      {/* スマホ版 フローティングトップバー */}
      <header className="sm:hidden fixed top-4 left-4 right-4 h-14 px-4 z-10 bg-white/95 dark:bg-[#252525]/95 backdrop-blur-md rounded-2xl shadow-md border border-gray-100 dark:border-neutral-800 flex items-center justify-between">
      ...
      {/* ワークスペース */}
      <div className="flex-1 overflow-y-auto relative scrollbar-hide pt-20 sm:pt-0 pb-24 sm:pb-0">
        {mode === 'edit' ? (
          <div className="max-w-4xl mx-auto min-h-full p-4 sm:p-12 relative flex flex-col">
            {/* 余白ゆったりの無地エディタ */}
            <div className="flex-1 bg-white dark:bg-[#1e1e1e] rounded-2xl shadow-sm border border-gray-200 dark:border-neutral-800 overflow-hidden relative">
              <textarea 
                ref={textareaRef}
                value={localText}
                ...
                className="w-full h-full min-h-[60vh] resize-none outline-none bg-transparent text-gray-800 dark:text-[#e5e5e5] px-6 sm:px-14 py-12 sm:py-16 placeholder-gray-300 dark:placeholder-neutral-600"
          ...
          <div className="min-h-full p-4 sm:p-12 flex justify-center items-start">
            <div 
              id="preview-canvas"
              className="p-8 sm:p-16 shadow-lg transition-colors duration-300 ease-in-out bg-white dark:bg-[#191919]"
              style={{
      ...
      {/* フローティングツールバー */}
      {mode === 'edit' && (
        <div className="absolute bottom-20 sm:bottom-8 left-1/2 transform -translate-x-1/2 bg-white/95 dark:bg-neutral-800/95 backdrop-blur-md shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-200 dark:border-neutral-700 rounded-full px-2 sm:px-4 py-2 flex items-center space-x-1 sm:space-x-2 z-20">
      ...
      {/* スマホ専用 ボトムナビゲーション */}
      <nav className="sm:hidden fixed bottom-0 left-0 w-full h-16 bg-white/95 dark:bg-[#191919]/95 backdrop-blur-md border-t border-gray-200 dark:border-neutral-800 z-30 flex items-center justify-around px-2 pb-safe">
        <button onClick={() => setMode('edit')} className={`flex flex-col items-center justify-center w-[30%] h-full space-y-1 ${mode === 'edit' ? 'text-gray-900 dark:text-white' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}>
          <PenLine size={20} strokeWidth={mode === 'edit' ? 2.5 : 2} />
          <span className="text-[10px] font-bold">執筆</span>
        </button>
        <button onClick={() => setMode('preview')} className={`flex flex-col items-center justify-center w-[30%] h-full space-y-1 ${mode === 'preview' ? 'text-gray-900 dark:text-white' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}>
          <Eye size={20} strokeWidth={mode === 'preview' ? 2.5 : 2} />
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {bgColors.map(color => (
                      <button key={color} onClick={() => updateSetting('backgroundColor', color)} className={`w-8 h-8 rounded-full border-2 transition-transform hover:scale-110 ${settings.backgroundColor === color ? 'border-gray-900 dark:border-white scale-110 shadow-md' : 'border-gray-300 dark:border-neutral-600'}`} style={{ backgroundColor: color }} />
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {textColors.map(color => (
                      <button key={color} onClick={() => updateSetting('textColor', color)} className={`w-8 h-8 rounded-full border-2 transition-transform hover:scale-110 ${settings.textColor === color ? 'border-gray-900 dark:border-white scale-110 shadow-md' : 'border-gray-300 dark:border-neutral-600'}`} style={{ backgroundColor: color }} />
                    ))}
                  </div>
                </div>
              </div>
      ...
      {/* スマホ版 フルスクリーンメニュー */}
      {isMobileMenuOpen && (
        <div className="sm:hidden fixed inset-0 bg-white dark:bg-[#121212] z-50 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-200">
          <header className="h-14 flex items-center justify-between px-4 border-b border-gray-100 dark:border-neutral-800 bg-white/80 dark:bg-[#121212]/80 backdrop-blur-md">
            <h2 className="font-display font-bold text-lg dark:text-white">メニュー</h2>
            <button onClick={() => setIsMobileMenuOpen(false)} className="text-gray-500 dark:text-gray-400 p-2 rounded-full hover:bg-gray-100 dark:hover:bg-neutral-800">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-8 pb-24">
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={handleExportClick} className="flex flex-col items-center justify-center space-y-2 bg-gray-50 dark:bg-neutral-900 p-4 rounded-2xl border border-gray-100 dark:border-neutral-800 active:scale-95 transition-transform">
                  <Download size={24} className="text-gray-700 dark:text-gray-300" />
                  <span className="text-sm font-bold dark:text-gray-200">画像化</span>
                </button>
                <button onClick={splitForX} className="flex flex-col items-center justify-center space-y-2 bg-gray-50 dark:bg-neutral-900 p-4 rounded-2xl border border-gray-100 dark:border-neutral-800 active:scale-95 transition-transform">
            ...
            <section className="space-y-3">
              ...
              <div className="space-y-4 bg-gray-50 dark:bg-neutral-900 p-5 rounded-2xl border border-gray-100 dark:border-neutral-800">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold dark:text-gray-300">文字の大きさ</span>
                  <div className="flex space-x-2">
                    {fontSizes.map(fs => (
                      <button key={fs.value} onClick={() => updateSetting('editorFontSize', fs.value)} className={`px-4 py-2 text-xs font-bold rounded-lg ${settings.editorFontSize === fs.value ? 'bg-gray-900 text-white dark:bg-white dark:text-gray-900' : 'bg-gray-200 text-gray-600 dark:bg-neutral-800 dark:text-gray-400'}`}>{fs.label}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-3">
              ...
              <div className="space-y-5 bg-gray-50 dark:bg-neutral-900 p-5 rounded-2xl border border-gray-100 dark:border-neutral-800">
                <select value={settings.fontFamily} onChange={(e) => updateSetting('fontFamily', e.target.value)} className="w-full text-sm font-bold bg-white dark:bg-[#1a1a1a] dark:text-[#e5e5e5] border border-gray-200 dark:border-neutral-700 rounded-xl p-4 outline-none cursor-pointer">
                  <option value="var(--font-zen-kaku)">Zen Kaku Gothic New (角ゴシック)</option>
                  <option value="var(--font-zen-maru)">Zen Maru Gothic New (丸ゴシック)</option>
                  <option value="var(--font-zen-mincho)">Zen Old Mincho (明朝)</option>
                  <option value="var(--font-zen-kurenaido)">Zen Kurenaido (手書き)</option>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex space-x-2">
                    {fontSizes.map(fs => (
                      <button key={fs.value} onClick={() => updateSetting('fontSize', fs.value)} className={`px-4 py-2 text-xs font-bold rounded-lg ${settings.fontSize === fs.value ? 'bg-gray-900 text-white dark:bg-white dark:text-gray-900' : 'bg-gray-200 text-gray-600 dark:bg-neutral-800 dark:text-gray-400'}`}>{fs.label}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {bgColors.map(color => (
                      <button key={color} onClick={() => updateSetting('backgroundColor', color)} className={`w-10 h-10 rounded-full border-2 transition-transform hover:scale-110 ${settings.backgroundColor === color ? 'border-gray-900 dark:border-white scale-110 shadow-md' : 'border-gray-300 dark:border-neutral-700'}`} style={{ backgroundColor: color }} />
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {textColors.map(color => (
                      <button key={color} onClick={() => updateSetting('textColor', color)} className={`w-10 h-10 rounded-full border-2 transition-transform hover:scale-110 ${settings.textColor === color ? 'border-gray-900 dark:border-white scale-110 shadow-md' : 'border-gray-300 dark:border-neutral-700'}`} style={{ backgroundColor: color }} />
                    ))}
                  </div>
                </div>
              </div>
